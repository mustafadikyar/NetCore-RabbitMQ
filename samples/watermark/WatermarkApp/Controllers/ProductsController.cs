...
namespace WatermarkApp.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name,Price,Stock,ImageName")] Product product, IFormFile file)
        {
            if (!ModelState.IsValid) return View(product);

            if (file is { Length: > 0 })
            {
                string randomImageName = Guid.NewGuid() + Path.GetExtension(file.FileName);
